...
import clientlib.debuglib
import sys

address = "192.168.0.111:9576"
if __name__ == "__main__":
    requrl=""; uname='';passwd='';cookie=''
    ...
    if clientlib.lib.getres(conn,clientlib.lib.testurl) != b'csuccess':
        ...
        exit(1)
    # print(clientlib.lib.getres(conn,"/mission/consult/"))
    cookie = clientlib.lib.login(conn)
    ...
    while i:
        if i == '1':
            ...
            if newpasswd == newpasswdagain :
                clientlib.lib.changepasswd(conn,cookie,newpasswd)
            else:
        ...
        if i == '2':
            ...
            print(",'0' to exit\n")
            if clientlib.lib.opreate(conn,cookie) :
                sys.exit(0)
        i=input()
    sys.exit(0)
